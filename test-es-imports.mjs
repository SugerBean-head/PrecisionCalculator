...
(async () => {
  ...
  await test('命名导入', async () => {
    const { PrecisionCalculator, calc } = await import('./precision-calculator.mjs');
  ...
  await test('默认导入', async () => {
    const precisionCalc = (await import('./precision-calculator.mjs')).default;
  ...
  await test('命名空间导入', async () => {
    const PC = await import('./precision-calculator.mjs');
  ...
  await test('链式调用', async () => {
    const { ChainableCalculator } = await import('./precision-calculator.mjs');
  ...
  await test('批量计算', async () => {
    const { batch } = await import('./precision-calculator.mjs');
  ...
  await test('版本信息', async () => {
    const { version } = await import('./precision-calculator.mjs');
  ...
  await test('功能特性', async () => {
    const { features } = await import('./precision-calculator.mjs');
  ...
  await test('动态导入', async () => {
    const { calc } = await import('./precision-calculator.mjs');
  ...
  if (testsPassed === testsTotal) {
    ...
    console.log('\n✨ 支持的导入方式:');
    console.log('   • import { calc } from "precision-calculator"');
    console.log('   • import precisionCalc from "precision-calculator"');
    console.log('   • import * as PC from "precision-calculator"');
    console.log('   • import { PrecisionCalculator as Calc } from "precision-calculator"');
    process.exit(0);
  } else {
    console.log('❌ 部分测试失败');
    process.exit(1);
